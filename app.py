...
📊 DPDzero Productivity Dashboard
</h1>
...
if view_type == "👤 Advisor View" and advisor_data is not None:
    ...
    # LOP card removed - now 6 KPI cards
    ...
    with c6: card("🎯 Performance", f"{advisor_data['Performance (%)']}%")
    ...
    chart_df = pd.DataFrame({
        "KPI": ["Attendance", "Performance", "Productivity", "Compliance"],
        "Score": [
...
# ===================================================
    ...
    with c6: card("🎯 Avg Performance", f"{round(team_df['Performance (%)'].astype(float).mean(), 2)}%")
    ...
    score_cols = ["Attendance Score (1-5)", "Productiviy Score (1-5)", "Compliance Score (1-5)", "Performance Score (1-5)"]
    team_scores = team_df[score_cols].astype(float).mean()
    team_chart_df = pd.DataFrame({
        "Metric": ["Attendance", "Productivity", "Compliance", "Performance"],
        "Avg Score": [
            team_scores["Attendance Score (1-5)"],
            team_scores["Productiviy Score (1-5)"], team_scores["Compliance Score (1-5)"],
            team_scores["Performance Score (1-5)"]
        ]
    })
...
# ===================================================
    ...
    # (LOP metric removed - final score now balanced across 5 metrics)
    def get_aggregated_summary(df, group_by_col):
        ...
        summary = df.groupby(group_by_col).agg(
            ...
            Avg_Performance_Pct=('Performance (%)', 'mean'),
        ).reset_index()

        # Calculate final balanced rating out of 5 across all 5 metrics cleanly
        ...
        perf_score = (summary['Avg_Performance_Pct'] / 100) * 5

        # Final Rating is balanced across 5 metrics (Star Rating + 4 scaled percentages)
        summary['Final_Performance_Rating_out_of_5'] = (
            summary['Avg_Star_Rating'] + prod_score + comp_score + att_score + perf_score
        ) / 5
        ...
        summary = summary.rename(columns={
            ...
            "Avg_Performance_Pct": "Avg Performance (%)",
